...
export default function CategoryContent({ initialProducts = [], categorySlug }: CategoryContentProps) {
  ...
  const getInitialCategoryName = () => {
    if (initialProducts.length > 0 && initialProducts[0].category) {
      return typeof initialProducts[0].category === 'object' ? initialProducts[0].category.name : initialProducts[0].category;
    }
    return categorySlug.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
  };
  
  const [categoryName, setCategoryName] = useState<string>(getInitialCategoryName);
  ...
  const fetchProducts = async () => {
    // Silent background fetch to check for updates
    try {
