...
export default function CategorySlider() {
  ...
  const categories = useMemo(() => {
    return allCategories.filter((c) => (c.products_count ?? 0) > 0);
  }, [allCategories]);
  ...
  const handlePrev = () => {
    if (!carouselRef.current) return;
    let newPos = carouselRef.current.scrollLeft - 240;
    if (newPos < 0) newPos = categories.length * 160;
    setCurrentIndex(Math.floor(newPos / 160));
    carouselRef.current.scrollTo({ left: newPos, behavior: 'smooth' });
  };
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-6 border-b border-[#052326]/10 pb-6">
        <div>
          ...
          <p className="text-sm text-[#052326]/80 mt-2 max-w-xl font-light">
            Explore authentic herbal extracts, CBD oils, wellness capsules, and premium organic remedies.
          </p>
        </div>
                      ...
                      src={
                        category.image
                          ? (category.image.startsWith('http')
                            ? category.image
                            : `${process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:8000'}${category.image.startsWith('/') ? '' : '/storage/'}${category.image}`)
                          : '/fallback.png'
                      }
